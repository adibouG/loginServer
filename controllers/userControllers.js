    ...
    try {
        ...
        if (!results.rows.length) { //no result or wrong attempt
          if (!USE_LOGIN_TMP_BLOCK) throw new EnzoError.InvalidRequest(401, MESSAGES.ERR_WRG_CRD, userData);
            ...
            resObj = handleFailedLoginAttempt() ;
            return res.status(resObj.status).send(resObj);  
          }
        } else {
            let userObject = null;
            ...
            */
            if (!USE_OAUTH_SERVER) {
                ...
                /* OR DEPENDS ON OAUTH SERVER SETTING DURING TESTING */
                let data = {} ;
...
const handleFailedLoginAttempt = (userAttempts) => {
  let resObj;
...
const requestRecoveryEmail =  async (req, res) => {
  ...
  try {
      ...
      resObj = new utilities.ResponseBody(200, MESSAGES.PWDRESET_EMAIL_LINK_SEND, email);
      return res.status(resObj.status).send(resObj) ; 
  } catch(err) {
      console.log(err);
      resObj = new utilities.ResponseBody(500, MESSAGES.SERVERERROR, email);
      return res.status(resObj.status).send(resObj) ; 
